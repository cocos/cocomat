                                        ...
                                        const char *blacklist)
{
    ...
    ret = av_mallocz_array(url_protocols_count, sizeof(*ret));
    if (!ret)
        return NULL;
...
int avformat_protocols_deinit(void){
    if(!url_protocols_initialized) return -1;
    url_protocols_initialized = 0;
    url_protocols_count = url_protocols_init_count;
    av_freep(&url_protocols);
    return 0;
}

int avformat_register_protocol(const URLProtocol* protocol){
    if(!url_protocols_initialized) return -1;
    int count = url_protocols_count + 1;
    const URLProtocol **protocols = av_realloc_array(url_protocols, count, sizeof(URLProtocol*));
    if (!protocols)
        return AVERROR(ENOMEM);
    protocols[count - 2] = protocol;
    protocols[count - 1] = NULL;
    url_protocols = protocols;
    url_protocols_count ++;
    return 0;
}
